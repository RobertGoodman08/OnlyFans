...
def index(request):
    user = request.user
    stream_items = Stream.objects.filter(user__exact=user).order_by('-date')
...
def get_search(request):
    if request.method == 'POST':
        ...
        users_search = User.objects.filter(username__contains=searched)


        return render(request, 'search.html', {'searched': searched, 'users_search': users_search})
    else:
