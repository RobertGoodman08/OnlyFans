...
from users.forms import NewListForm
    ...
    if request.user != user:
        try:
            ...
            if url == 'profilephotos':
                posts = PostFileContent.objects.filter(user=user,
                ...
                posts = Post.objects.filter(user=user, tier__number__lte=subscriber_tier.tier.number).order_by(
                    '-posted')
                page_type = 3
        except Exception:
